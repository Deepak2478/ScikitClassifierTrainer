...
import com.pavoindus.modeltraining.repository.*;
...
public class ModelTrainingServiceImpl implements ModelTrainingService {
    ...
    @Override
    public ModelDetails getModel(Long id) {
        return getModelDetailsByModelId(id);
    }
    ...
    public APIResponse getModelAnalysisFromPredictiveService(MultipartFile file, Long modelId) {
        ...
        try {
            ...
            data = new String[rows.length][28];
            int index = 0;
            for(String row : rows) {
                String[] columns = row.split(",");
                logger.info("Processing row: " + index);
                if(columns.length != 28) {
                    logger.error("Row doesn't contain 28 cols. Rejecting: " + row);
                    sb.append("Row doesn't contain 28 cols. Rejecting: ");
                    sb.append(row);
                    sb.append(", ");
                    hasErrors = true;
                }
    ...
    private ModelDetails getModelDetailsByModelId(Long modelId) {
        Model model = modelRepository.findOne(modelId);
        return getModelDetailsByModel(model);
    }
    private ModelDetails getModelDetailsByModel(Model model) {
        if(model == null) {
            return null;
        }
        ModelConfig config = modelConfigRepository.findOne(model.getId());
        ModelAnalysis analysis = modelAnalysisRepository.findOne(model.getId());
        return new ModelDetails(model, config, analysis);
    }
}
