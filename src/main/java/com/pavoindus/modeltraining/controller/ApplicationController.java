...
public class ApplicationController {
    ...
    public @ResponseBody APIResponse getAllModels() {
        List<ModelDetails> models = modelTrainingService.getAllModels();
    ...
    public @ResponseBody APIResponse getModelConfig(@PathVariable("id") Long id) {
        ModelDetails modelDetails = modelTrainingService.getModel(id);
        if(modelDetails == null) {
            return new Failure("No model exists with ID: " + id);
        }
        return new Success(modelDetails);
    }

    @PostMapping("/model/{id}/train")
    public @ResponseBody APIResponse trainModel(@PathVariable("id") Long id) {
        ModelDetails details = modelTrainingService.getModel(id);
        if(details == null) {
            return new Failure("No model exists with ID: " + id);
        }
        modelTrainingService.queueModelForTraining(details.getConfig());
        return new Success();
    }
    @GetMapping("/train")
